...
function cashIn() {
    ...
    if (amount && !isNaN(amount)) {
        ...
        history.push({ type: 'Cash In', amount: parseFloat(amount), date: new Date() });
        updateTotalDisplay();
        saveToLocalStorage();
    }
}
...
function cashOut() {
    ...
    if (amount && !isNaN(amount)) {
        total -= parseFloat(amount);
        history.push({ type: 'Cash Out', amount: parseFloat(amount), date: new Date() });
        updateTotalDisplay();
        saveToLocalStorage();
    }
}
...
function toggleHistory() {
    ...
    history.forEach(entry => {
        ...
        item.textContent = `${entry.type}: $${entry.amount} - ${entry.date.toLocaleString()}`;
